	...
	<div class="information container">
		...
		<div class="table-responsive" style="overflow: hidden;">
			...
			<s:else>
				...
				<td>
					<span class="companyName" title="<s:property value="#comm.comment_company.companyName" />" style="cursor: pointer;"><s:property value="#comm.comment_company.companyName" /></span>
				</td>
				<td>
			...
			<s:if test="#request.page==1">
				...
				<s:else>
					...
					<a class="btn btn-default" >上一页</a>&nbsp;
					第<s:property value="#request.page"/>页&nbsp;
					<a class="btn btn-success" href="<%=path%>/user/Comment_selectUserAllComment.action?page=<s:property value="#request.page+1"/>">下一页</a>&nbsp;
					<s:if test="#request.counts%10==0">
						<a class="btn btn-info" href="<%=path%>/user/Comment_selectUserAllComment.action?page=<s:property value="#request.counts/10"/>">尾页</a>
					</s:if>
					<s:else>
						<a class="btn btn-dinfo" href="<%=path%>/user/Comment_selectUserAllComment.action?page=<s:property value="#request.counts/10+1"/>">尾页</a>
					</s:else>
				</s:else>
    		</s:if>
			<s:else>
				<s:if test="#request.page*10>=#request.counts" >
					<a class="btn btn-info" href="<%=path%>/user/Comment_selectUserAllComment.action?page=1">首页</a>&nbsp;
					<a class="btn btn-success" href="<%=path%>/user/Comment_selectUserAllComment.action?page=<s:property value="#request.page-1"/>">上一页</a>&nbsp;
					第<s:property value="#request.page"/>页&nbsp;
					<a class="btn btn-default" >下一页</a>&nbsp;
					<a class="btn btn-default" >尾页</a>
				</s:if>
				<s:else>
					...
					<s:else>
						<a class="btn btn-infot" href="<%=path%>/user/Comment_selectUserAllComment.action?page=<s:property value="#request.counts/10+1"/>">尾页</a>
					</s:else>
				</s:else>
			</s:else>
		</div>
