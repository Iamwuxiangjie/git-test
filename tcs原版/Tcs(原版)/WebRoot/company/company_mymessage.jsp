       ...
       <nav class="navbar navbar-default navbar-fixed-top">
        ...
        <div class="information container">
    		...
    		<form method="post" id="form1">
				...
				<tr>
					<td><input class="message1" type="checkbox" name="messageidlist" value="<s:property value="#message.messageId"/>" onclick="check('message1')" ></td>
					<td>
					...
					<td>
						...
						<s:if test="#message.messageFromUser!=null||#message.messageFromAdmin!=null||#message.messageFromCompany!=null">
							<a href="<%=path%>/company/Message_getMessageToPeople?msgid=<s:property value="#message.messageId"/>&key=<s:property value="#request.key"/>">回复</a>
						</s:if>
						<a href="<%=path%>/company/Message_readMessage.action?messageidlist=<s:property value="#message.messageId"/>&page=<s:property value="#request.page"/>&key=<s:property value="#request.key"/>">已读</a>
						<a href="<%=path%>/company/Message_deleteMessage.action?messageidlist=<s:property value="#message.messageId"/>&page=<s:property value="#request.page"/>&key=<s:property value="#request.key"/>"
            ...
            <form action="<%=path%>/company/Message_selectComAllMessage.action?page=1" method="get">
                <div class="input-group">
		...
		<s:else>
		<form action="<%=path%>/company/Message_deleteMessage.action?page=<s:property value="#request.page"/>&key=<s:property value="#request.key"/>" method="post">
		<table class="table table-hover">
			<thead>
			<tr>
			...
			<s:iterator value="#request.readmessage" var="message">
				<tr>
					<td><input class="message2" type="checkbox" name="messageidlist" value="<s:property value="#message.messageId"/>" onclick="check('message2')"></td>
					<td>
						<s:if test="#message.messageFromCompany!=null">
							服务公司：<s:property value="#message.messageFromCompany.companyName" />
						</s:if>
					...
					<td>
						...
						<a href="#" class="check" data-toggle="modal" data-target="#myModal" onclick="replace(<s:property value="#message.messageId"/>)">查看</a>
						<s:if test="#message.messageFromUser!=null||#message.messageFromAdmin!=null||#message.messageFromCompany!=null">
							<a href="<%=path%>/company/Message_getMessageToPeople.action?msgid=<s:property value="#message.messageId"/>&page=<s:property value="#request.page"/>&key=<s:property value="#request.key"/>">回复</a>
						</s:if>
						<a href="<%=path%>/company/Message_deleteMessage.action?messageidlist=<s:property value="#message.messageId"/>&page=<s:property value="#request.page"/>&key=<s:property value="#request.key"/>"
						onclick="javascript: return confirm('真的要删除吗？');">删除</a>
					</td>
		...
		<s:if test="#request.page==1">
			...
			<s:else>
				...
				<a href="<%=path%>/company/Message_selectComAllMessage.action?page=<s:property value="#request.page+1"/>&key=<s:property value="#request.key"/>">下一页</a>&nbsp;
				<s:if test="#request.counts%10==0">
					<a href="<%=path%>/company/Message_selectComAllMessage.action?page=<s:property value="#request.counts/10"/>&key=<s:property value="#request.key"/>">尾页</a>
				</s:if>
				<s:else>
					<a href="<%=path%>/company/Message_selectComAllMessage.action?page=<s:property value="#request.counts/10+1"/>&key=<s:property value="#request.key"/>">尾页</a>
				</s:else>
			</s:else>
    	</s:if>
		<s:else>
			<s:if test="#request.page*10>=#request.counts" >
				<a href="<%=path%>/company/Message_selectComAllMessage.action?page=1&key=<s:property value="#request.key"/>">首页</a>&nbsp;
				<a href="<%=path%>/company/Message_selectComAllMessage.action?page=<s:property value="#request.page-1"/>&key=<s:property value="#request.key"/>">上一页</a>&nbsp;
...
<script type="text/javascript">
	...
	var form1=document.getElementById('form1');
		if(ev.value=='批量删除')
			form1.action='<%=path%>/company/Message_deleteMessage.action?page=<s:property value="#request.page"/>&key=<s:property value="#request.key"/>';
		else if(ev.value=='批量已读')
			form1.action='<%=path%>/company/Message_readMessage.action?page=<s:property value="#request.page"/>&key=<s:property value="#request.key"/>';
	}
</script>
	<%-- <s:fielderror />
	<h2>未读消息</h2>
	...
	<s:else>
		...
		<table border="1">
			...
			<s:iterator value="#request.unreadmessage" var="message">
				<tr>
					...
					<td><s:property value="#message.messageBody" />
					</td>
					<!-- 信息内容将隐藏，选中该行后显示在下面的详细信息中 -->
					<td><a
						href="<%=path%>/company/Message_readMessage.action?messageidlist=<s:property value="#message.messageId"/>&page=<s:property value="#request.page"/>&key=<s:property value="#request.key"/>">已读</a>
						<a
						href="<%=path%>/company/Message_deleteMessage.action?messageidlist=<s:property value="#message.messageId"/>&page=<s:property value="#request.page"/>&key=<s:property value="#request.key"/>"
						onclick="javascript: return confirm('真的要删除吗？');">删除</a>
						<s:if test="#message.messageFromUser!=null||#message.messageFromAdmin!=null||#message.messageFromCompany!=null">
							<a href="<%=path%>/company/Message_getMessageToPeople?msgid=<s:property value="#message.messageId"/>&key=<s:property value="#request.key"/>">回复</a>
						</s:if>
		...
		</table><!-- <%=path%>/company/Message_deleteMessage.action?page=<s:property value="#request.page"/> -->
			<input type="submit" value="批量删除" onclick="pd(this)"><input type="submit" value="批量已读" onclick="pd(this)">
		</form>
	</s:else>

	<h2>已读消息</h2>
	<form action="<%=path%>/company/Message_selectComAllMessage.action?page=1" method="get">
	请输入信息标题关键字<input type="text" name="key" value="${request.key }">
