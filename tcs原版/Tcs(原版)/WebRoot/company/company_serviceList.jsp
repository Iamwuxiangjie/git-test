...
<!-- navbar -->
	...
	<div class="information container">
    	...
    	没有服务
			...
			<s:else>
				...
				<tr>
					<td><s:property value="#ser.serviceName" />
					</td>
			...
			<s:else>
				...
				<a href="<%=path%>/company/Service_selectComAllService.action?page=<s:property value="#request.page+1"/>&key=<s:property value="#request.key"/>">下一页</a>&nbsp;
				<s:if test="#request.counts%10==0">
					<a href="<%=path%>/company/Service_selectComAllService.action?page=<s:property value="#request.counts/10"/>&key=<s:property value="#request.key"/>">尾页</a>
				</s:if>
				<s:else>
					<a href="<%=path%>/company/Service_selectComAllService.action?page=<s:property value="#request.counts/10+1"/>&key=<s:property value="#request.key"/>">尾页</a>
				</s:else>
			</s:else>
    	</s:if>
		<s:else>
			<s:if test="#request.page*10>=#request.counts" >
				<a href="<%=path%>/company/Service_selectComAllService.action?page=1&key=<s:property value="#request.key"/>">首页</a>&nbsp;
				<a href="<%=path%>/company/Service_selectComAllService.action?page=<s:property value="#request.page-1"/>&key=<s:property value="#request.key"/>">上一页</a>&nbsp;
	...
	<%-- <form action="<%=path%>/company/Service_selectComAllService.action?page=1" method="get">
	请输入服务名称关键字<input type="text" name="key" value="${request.key }">
