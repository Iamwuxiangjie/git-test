...
<html>
  ...
  <!-- navbar -->
  	...
  	<%-- <s:if test="#session.loginUser!=null">
  		欢迎!${sessionScope.loginUser.userName}&nbsp;<a href="<%=path%>/user/user_login_success.jsp">个人中心</a>
  	</s:if>
  	<s:elseif test="#session.loginCompany!=null">
  		欢迎!${sessionScope.loginCompany.companyName}&nbsp;<a href="<%=path%>/company/company_login_success.jsp">管理中心</a>
  	</s:elseif>
  	<s:else>
  		欢迎游客！要预约服务请先<a href="<%=path%>/web/login.jsp">登录</a>
  	</s:else>
  	<a href="<%=path%>/web/index.jsp">首页</a>
  	<div>
  		<a <s:if test="#ServiceTypeId==0">class="afteraction"</s:if>
  		href="user/Service_getLikeServices?page=1&ServiceTypeId=0">全部</a>
  			<s:iterator var="type" status="num" value="#request.servicetype">
				<a <s:if test="#ServiceTypeId==#type.serviceTypeId">class="afteraction"</s:if>
				 href="user/Service_getLikeServices?page=1&ServiceTypeId=<s:property value='#type.serviceTypeId'/>"><s:property value="#type.serviceTypeName"/></a>	
  			</s:iterator>
  		<form method="post" action="user/Service_getLikeServices">
  			<input type="hidden" name="page" value="1"/>
  			<input type="hidden" name="ServiceTypeId" value="<s:property value='#ServiceTypeId==null?0:#ServiceTypeId'/>"/>	
  			<input type="text" name="likename" value="<s:property value="#likename"/>"/>
    ...
    <table border="1px">
    	...
    	<s:iterator var="service" status="num" value="#serviceslist">
    	<tr>
    		<td><s:property value="#num.index+1"/></td>
    		<td><s:property value="#service.serviceName"/></td>
    		<td><s:property value="#service.serviceExplain"/></td>
    		<td>
    			<s:if test="#service.serviceImg!=null">
    				<img width="50" height="50" alt="服务图片" src="<s:property value="#service.serviceImg"/>">
    			</s:if>
    			<s:else>
    				暂无服务图片
    			</s:else>
    		...
    		<td><s:property value="#service.serviceprice"/></td>
    		<td><s:property value="#service.service_company.companyName"/></td>
    		<td><s:property value="#service.service_priveunit.priveunitName"/></td>
    		<td><s:property value="#service.service_serviceType.serviceTypeName"/></td>	
    		<td><s:property value="#service.service_company.companyServiceScope"/></td>	
    		<td>
    		<form method="get" action="web/Service_forwardto">
    			<input type="hidden" value="<s:property value="#service.serviceId"/>" name="serviceid"/>
    			<input type="hidden" value="1" name="page"/>
    	...
    	<tr>
    		<td colspan=“10”>
    			<s:if test="#nowpage>1">
    				<a href="user/Service_getLikeServices?page=<s:property value='#nowpage-1'/>&ServiceTypeId=<s:property value='#ServiceTypeId'/>&likename=<s:property value='likename'/>">上一页</a>
    			</s:if>
    			<s:if test="#nowpage<#themaxpage">
    				<a href="user/Service_getLikeServices?page=<s:property value='#nowpage+1'/>&ServiceTypeId=<s:property value='#ServiceTypeId'/>&likename=<s:property value='likename'/>">下一页</a>
    			</s:if>
    		</td>
    	</tr>
    </table> --%>
