...
<script type="text/javascript">
  ...
  <%-- <script type="text/javascript">
  	function numchanges(a){
  		...
  		var but_reduce=document.getElementById("but_reduce");
  		if(parseInt(numvalue.innerHTML)>1){
  			but_reduce.style.display='block';
  		}else{
  			but_reduce.style.display='none';
  		}
  	}
  	function addOrder(){
  		var result=confirm("你确定要提交");
  		if(result){
  			var numvalue=document.getElementById("NumOfOrder");
  			var num=parseInt(numvalue.innerHTML);
  			document.getElementById('theNumOfOrder').value=num;
  ...
  <br>
  <a href="<%=path%>/user/Service_getLikeServices?page=1">返回</a>
  <table border="1">
   <tr>
    		<td>服务名字</td>
	    	<td>服务简介</td>
    	...
    	<tr>
    		...
    		<td>
    			<s:if test="#service.serviceImg!=null">
    				<img width="100" height="100" alt="服务图片" src="<s:property value="#service.serviceImg"/>">
    			</s:if>
    			<s:else>
    				暂无服务图片
    			</s:else>
    		...
    		<s:if test="#session.loginUser!=null">
   			<td>
   				<div>数量x</div>
   				<button onclick="numchanges('-')" id="but_reduce">-</button>
   				<div id="NumOfOrder">1</div>
   				<button onclick="numchanges('+')">+</button>
   				<form  action="user/Order_addOrder" method="post" onsubmit="return addOrder()">
    				<input type="hidden" name="companyId" value="<s:property value='#service.service_company.companyId'/>"/>
    				<input type="hidden" name="serviceId" value="<s:property value='#service.serviceId'/>"/>
    				<input type="hidden" name="theNumOfOrder" value="1" id="theNumOfOrder"/>
    ...
    <table border="1">
    	...
    	<tr>
    		...
    		<td><s:property value="#comment.commentCont"/></td>
    		<td><s:property value="#comment.commentTime"/></td>
    		<td><s:property value="#comment.commentGrade"/></td>
    	</tr>
    	</s:iterator>
    	<tr>
    		<td colspan="4">
